...
void PassII(char* buf)
{
	...
	default: // 명령어
	{
		...
		if (!_stricmp(Instr[i].dest, "m")) {
			...
			sprintf(Sen.operand[0], "%d", Symbol[symbolIndex].lc);
		}
		else if (!_stricmp(Instr[i].sour, "m")) {
			symbolIndex = findSymbolIndex(Sen.operand[1]);
			sprintf(Sen.operand[1], "%d", Symbol[symbolIndex].lc);
			printf("| %s | ", Symbol[symbolIndex].word_type);
			Instr[0].word_type == Symbol[symbolIndex].word_type;
		}

		// 출력
		printf("%02x ", btoi(tempModReg));

		if (!_stricmp(Instr[i].sour, "i") || !_stricmp(Instr[i].sour, "m")) {
			printf("| %s | ", Instr[0].word_type);
			int length = (Instr[0].word_type[0] == 'w') ? 4 : 2;
			printf("%0*x", length, (unsigned short)atoi(Sen.operand[1]));
		}
		else {
			printf("%s", Instr[i].ins_code);
		}

		//if(symbolIndex != -1) printf(" %s", Symbol[symbolIndex].symbol);
		printf("\n");
